...
public class HAB implements Listener {
	...
	private static void d(EntityDamageEvent paramEntityDamageEvent) {
		...
		if ((KitAPI.Stomper.contains(localPlayer1.getName()))
				&& (paramEntityDamageEvent.getCause() == EntityDamageEvent.DamageCause.FALL)) {
			Iterator<Entity> localIterator = localPlayer1.getNearbyEntities(6.0D, 4.0D, 6.0D).iterator();
			Object localObject;
			...
			for (double d1 = -1.0D; d1 <= 1.0D; d1 += 1.0D) {
				for (double d2 = -1.0D; d2 <= 1.0D; d2 += 1.0D) {
					localObject = new Location(localPlayer1.getWorld(), localPlayer1.getLocation().getX() + d1,
							localPlayer1.getLocation().getY(), localPlayer1.getLocation().getZ() + d2);
					for (int i = 0; i < 3; i++) {
	...
	public void interact(PlayerInteractEvent e) {
		...
		if ((KitAPI.Kangaroo.contains(p.getName())) && (p.getItemInHand().getType() == Material.FIREWORK)) {
			e.setCancelled(true);
	...
	public void a1(PlayerToggleSneakEvent paramPlayerToggleSneakEvent) {
		Player localPlayer1 = paramPlayerToggleSneakEvent.getPlayer();
		if ((paramPlayerToggleSneakEvent.isSneaking()) && (KitAPI.Ajnin.contains(localPlayer1.getName()))
				&& (a.containsKey(localPlayer1))) {
			Player localPlayer2 = (Player) a.get(localPlayer1);
			if ((localPlayer2 != null) && (!localPlayer2.isDead())) {
				String str = null;
	...
	public void onVacuum(PlayerInteractEvent event) {
		...
		if ((KitAPI.Vaccum.contains(p.getName())) && (p.getItemInHand().getType().equals(Material.EYE_OF_ENDER))) {
